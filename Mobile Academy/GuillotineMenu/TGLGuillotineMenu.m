...
//    NSBundle(*forClass: self.dynamicType).loadNibNamed("LoadingDialog", owner: self, options: nil)[0] as! UIView

//    menuView = [[UIView alloc] initWithFrame:CGRectMake(0.0, 0.0, screenW, screenH)];
//    menuView.backgroundColor = menuColor;
    menuView = [[[NSBundle mainBundle] loadNibNamed:@"MenuView" owner:self options:nil] firstObject];
    menuView.frame = CGRectMake(0.0, 0.0, screenW, screenH);
    ...
    // - Menu Button
    float buttonMenuW = 25.0;
    float buttonMenuH = 25.0;
...
- (void)openMenu{
    ...
    if ( [(NSString*)[UIDevice currentDevice].model hasPrefix:@"iPad"] ) {
        vectorOpen = CGVectorMake(0, 5000.0);
    }
